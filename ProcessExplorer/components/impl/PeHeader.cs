...
namespace ProcessExplorer.components
{
    class PeHeader : PluginInterface.SuperHeader
    {
        ...
        public PeHeader(ProcessHandler processHandler, PluginInterface.DataStorage dataStorage, int startingPoint) : base("pe header", 8, 3)
        {
            ...
            string[] signatureHex = GetData(0, 1, PluginInterface.Enums.DataType.HEX, false, true, dataStorage).Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            int bit = int.Parse(GetBigEndianValue(GetData(1, 1, PluginInterface.Enums.DataType.HEX, false, true, dataStorage)), NumberStyles.HexNumber);
            dataStorage.Is64Bit = (bit == 0x8664);

            SectionAmount = int.Parse(GetData(2, 1, PluginInterface.Enums.DataType.DECIMAL, true, true, dataStorage)); // Sets the amount of sections
        ...
        public override void OpenForm(int row, PluginInterface.DataStorage dataStorage)
        {
            ...
            {
                string hexValue = GetData(row, 1, PluginInterface.Enums.DataType.HEX, true, false, dataStorage);
